 ...
 */
interface WinnieEdit {
}

/**
 * Define the document (the data model) used for Kenga views.
 */
class KengaViewDocument extends Disposable implements vscode.CustomDocument {
	...
	static async create(
		...
		return new KengaViewDocument(uri, fileData);
	}
	...
	private _edits: WinnieEdit[] = [];
	private _watcher: vscode.FileSystemWatcher;
	private _watchOnChange: vscode.Disposable;
	...
	private constructor(
		...
		// TODO: Somehow exclude the handling of the event if the change is produced by us while simple saving the document
		this._watchOnChange = this._watcher.onDidChange(e => {
			console.log(`The file ${uri.fsPath} has changed`);
		});
	}
	...
	private readonly _onDidChangeContent = this._register(new vscode.EventEmitter<{
		...
		readonly edits: readonly WinnieEdit[];
	}>());
	/**
	 ...
	 */
	makeEdit(edit: WinnieEdit) {
		this._edits.push(edit);

		this._onDidChange.fire({
			label: 'Stroke', // TODO: Substitute this with Winnie's edit name
			undo: async () => {
				this._edits.pop();
...
export class ViewEditorProvider implements vscode.CustomEditorProvider<KengaViewDocument> {
	...
	async openCustomDocument(
		...
		const document: KengaViewDocument = await KengaViewDocument.create(uri, openContext.backupId);
	...
	private onMessage(document: KengaViewDocument, message: any) {
		// TODO: Add edit driven messages to the Winnie Webview and mension them here
		// TODO: Add state-machine like messages from Winnie Webview here
		switch (message.type) {
			case 'editHappend':
				document.makeEdit(message as WinnieEdit);
				return;
		}
	}
}
